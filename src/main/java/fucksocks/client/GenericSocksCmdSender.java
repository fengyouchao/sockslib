 ...
 * @version 1.0
 */
public class GenericSocksCmdSender implements SocksCommandSender{
	...
	private CommandReplyMesasge checkServerReply(InputStream inputStream) 
		...
		logger.debug("{}", LogMessage.create(bufferReceived,
				bufferReceivedSize, MsgType.RECEIVE));
		...
		if(bufferReceived[3] == Socks5.ATYPE_IPV4){
			...
			logger.debug("Server replied:Address as IPv4:{}.{}.{}.{}, port:{}", a, b, c, d,
					 (UnsignedByte.toInt(portBytes[0]) << 8)
						| (UnsignedByte.toInt(portBytes[1]) ));
