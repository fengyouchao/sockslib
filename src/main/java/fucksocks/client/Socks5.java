...
public class Socks5 implements SocksProxy{
	 ...
	 */
	private SocksMethodRequestor socksMethodRequestor = 
			new GenericSocksMethodRequestor();

	/**
	 * Use to send command to SOCKS5 sever
	 */
	private SocksCommandSender socksCmdSender = new GenericSocksCmdSender();

	/**
	 * Resolve remote server's domain name in SOCKS server if it's false.
	 * It's default false.
	 */
	...
	public void buildConnection() throws SocksException, IOException {
		...
		SocksMethod method = socksMethodRequestor.doRequest(acceptableMethods, 
				proxySocket, SOCKS_VERSION);
		method.doMethod(this);
	}

	@Override
	public CommandReplyMesasge reqeustConnect(String host, int port) 
			throws SocksException, IOException{
		if(!alwaysResolveAddressLocally){
			// resolve address in SOCKS server
			return socksCmdSender.send(proxySocket, 
					SocksCommand.CONNECT, host, port, SOCKS_VERSION);
		}else{
			// resolve address in local.
			InetAddress address = InetAddress.getByName(host);
			return socksCmdSender.send(proxySocket, 
					SocksCommand.CONNECT, address, port, SOCKS_VERSION);
		}
	}

	@Override
	public CommandReplyMesasge reqeustConnect(InetAddress address, int port)
			throws SocksException, IOException {
		return socksCmdSender.send(proxySocket, SocksCommand.CONNECT, 
				address, port, SOCKS_VERSION);
	}

	@Override
	public CommandReplyMesasge reqeustConnect(SocketAddress address)
			throws SocksException, IOException {
		return socksCmdSender.send(proxySocket, SocksCommand.CONNECT, 
				address, SOCKS_VERSION);
	}
	...
	@Override
	public CommandReplyMesasge requestUdpAssociat(InetAddress address, int port) throws SocksException, IOException{
		return socksCmdSender.send(proxySocket, SocksCommand.UDP_ASSOCIATE,
				new InetSocketAddress(address, port), SOCKS_VERSION);
	}
	...
	@Override
	public SocksProxy setAuthentication(Authentication authentication) {
	...
	@Override
	public SocksProxy setSocksMethodRequestor(SocksMethodRequestor requestor) {
